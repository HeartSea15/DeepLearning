...
data_number = 10000;%训练特征样本数量=2000个循环×10个样本
line_number = 100;%50个数据作为一段
line_number_fft = 50; 
...
% fid=fopen（文件路径，‘打开方式’）
    ...
    % w后添加“b”，则以二进制格式打开
fid_fiture = fopen('..\..\mnist\50\train-images-idx3-ubyte','wb'); %特征
fid_label = fopen('..\..\mnist\50\train-labels-idx1-ubyte','wb'); %标签
...
%             f=1/T;
%             vecf=(0:N-1)*f;
%             subplot(2,2,1);plot(t,C);xlabel('时间（s）');ylabel('声压（pa）');title('原数据');
            
            C = awgn(C,SRN);%给正常信号添加信噪比为SRNdB的高斯白噪声
            
%             subplot(2,2,3);plot(t,C);xlabel('时间（s）');ylabel('声压（pa）');title('snr=20');
            C = C-mean(C); % 去直流
%             CR = CR -mean(CR);
            N=length(C);
            C = 2*abs(fft(C,N)/N);
...
%             CR = 2*abs(fft(CR,N)/N);
%             CR = CR(1:N/2);
%             subplot(2,2,2);plot(vecf(1:N/2),CR);xlabel('频率（f）');ylabel('声压（pa）');title('原数据');
%             subplot(2,2,4);plot(vecf(1:N/2),C);xlabel('频率（f）');ylabel('声压（pa）');title('snr=20');
