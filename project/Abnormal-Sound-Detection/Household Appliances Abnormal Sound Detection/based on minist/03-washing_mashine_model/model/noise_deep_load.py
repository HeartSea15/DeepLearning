...
test_accuracy, y_pred, y_test = sess.run([accuracy, tf.argmax(y_conv, 1), tf.argmax(y_, 1)],
                                 feed_dict={x: swallowsound.test.images,
                                            y_: swallowsound.test.labels,
                                            keep_prob: 0.5})
print('测试精度为： %g' % test_accuracy)  # 0.96119
print('测试结束！')
'''-------------------模型评估--------------------------------------------------'''
print('查准率precision: ', precision_score(y_test, y_pred))
print("查全率recall: ", recall_score(y_test, y_pred))
'''-------------------------------------------------------------------------------'''
