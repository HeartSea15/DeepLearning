...
def _read32(bytestream,MSB=True):
  if MSB:
    dt = numpy.dtype(numpy.uint32).newbyteorder('>')#大端排序
  else:
    dt = numpy.dtype(numpy.uint32).newbyteorder('<')#小端排序
...
class DataSet(object):
  ...
  def next_batch(self, batch_size, fake_data=False, shuffle=True):
    ...
    if fake_data:
      fake_image = [1] * 2500#需要改吗？
      if self.one_hot:
        fake_label = [1] + [0] * 75#需要改吗？
      else:
        fake_label = 0
